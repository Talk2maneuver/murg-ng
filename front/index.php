            ...
            if (mysqli_num_rows($stock_query) > 1) {
                ...
                if ($quantity > $available_stock) {
                    ...
                    if ($sql) {
                        ...
                        $qty = mysqli_query($con, "UPDATE stocks SET quantity='$remain' WHERE id='$actual_stock_id'");
                    } else {
                        ?>
                        <script>
                        ...
                        <div class="widget-content widget-content-area">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-3" id="custom-date-container" style="display: none;">
                                    ...
                                    <input type="date" class="form-control" id="from-date" onchange="applyFilters()">
                                </div>
                                <div class="col-md-3" id="custom-date-container-2" style="display: none;">
                                    <label>To Date:</label>
                                    <input type="date" class="form-control" id="to-date" onchange="applyFilters()">
                                </div>
                            </div>
                                ...
                                <table class="table table-hover" id="queue-table">
                                    ...
                                    <tbody>
                                        ...
                                        $queue_query = mysqli_query($con, "
                                            ...
                                            AND DATE(sq.creation) = '$today'
                                            AND sq.status = 'pending'
                                            ORDER BY sq.creation DESC
                                        ");
                                        if (mysqli_num_rows($queue_query) > 0) {
                                            while ($queue_row = mysqli_fetch_array($queue_query)) {
                                                ?>
                                                <tr>
                                                    <td><?php echo htmlspecialchars($queue_row['orderID']); ?></td>
                                                    <td><?php echo htmlspecialchars(!empty($queue_row['buyer_name']) ? $queue_row['buyer_name'] : $queue_row['customer_name']); ?></td>
                                                    <td>₦<?php echo number_format($queue_row['net_total']); ?></td>
                                                    <td><?php echo date('H:i', strtotime($queue_row['queue_time'])); ?></td>
                                                    <td>
                                                        <span class="badge badge-warning"><?php echo ucfirst($queue_row['status']); ?></span>
                                                    </td>
                                                    <td>
                                                        <button class="btn btn-sm btn-primary" onclick="viewReceipt('<?php echo $queue_row['orderID']; ?>')">View Receipt</button>
                                                    </td>
                                                </tr>
                                                <?php
                                            }
                                            ...
                                            ?>
                                            <tr>
                                                <td colspan="6" class="text-center">No pending sales in queue</td>
                                            </tr>
                                            <?php
                                        }
                                        ?>
...
function viewReceipt(orderID) {
    window.open('invoice?orderid=' + orderID);
}

// AJAX refresh for queue panel and badge
let lastPendingCount = <?php echo isset($pending_count) ? $pending_count : 0; ?>;

// Show/hide custom date inputs based on filter selection
$('#date-filter').on('change', function() {
    if ($(this).val() === 'custom') {
        $('#custom-date-container, #custom-date-container-2').show();
    } else {
        $('#custom-date-container, #custom-date-container-2').hide();
    }
});

// Apply filters and refresh queue
...
function refreshQueue() {
    ...
    let url = 'get_queue.php?filter=' + filter + '&status=' + status;
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.pending_count > 0) {
                if (badge.length === 0) {
                    $('.feather-bell').parent().append('<span class="badge badge-danger badge-up" id="queue-badge">' + response.pending_count + '</span>');
                } else {
                    badge.text(response.pending_count);
                }
            } else {
                badge.remove();
            }
            
            // Show notification if new items arrived
            if (response.pending_count > lastPendingCount) {
                Snackbar.show({
                    text: 'New sale received.',
                    duration: 3000,
                    pos: 'top-right',
            ...
            // Update queue panel table
            const tbody = $('#queue-table tbody');
            tbody.empty();
            
            if (response.queue_data.length > 0) {
                response.queue_data.forEach(function(item) {
                    const customerName = item.buyer_name || item.customer_name;
                    const row = `
                        <tr>
                            <td>${item.orderID}</td>
                            <td>${customerName}</td>
                            <td>₦${parseFloat(item.net_total).toLocaleString()}</td>
                            <td>${item.queue_time}</td>
                            <td>
                                <span class="badge badge-warning">${item.status.charAt(0).toUpperCase() + item.status.slice(1)}</span>
                            </td>
                            <td>
                                <button class="btn btn-sm btn-primary" onclick="viewReceipt('${item.orderID}')">View Receipt</button>
                            </td>
                        </tr>
                    `;
                    tbody.append(row);
                ...
                tbody.append('<tr><td colspan="6" class="text-center">No pending sales in queue</td></tr>');
            }
        }
    });
}

// Refresh queue every 25 seconds
$(document).ready(function() {
    setInterval(refreshQueue, 25000);
});

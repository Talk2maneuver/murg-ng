...
if (isset($_POST['cart'])) {
    ...
    if ($quantity > $available_stock) {
        ...
        if ($sql) {
            ...
            mysqli_query($con, "UPDATE stocks SET quantity='$new_quantity' WHERE id='$stockId'");
...
if (isset($_GET['del'])) {
    ...
    // Restore stock
    $stock_query = $con->query("SELECT quantity FROM stocks WHERE name='$item'");
    $stock_row = $stock_query->fetch_assoc();
    $new_quantity = $stock_row['quantity'] + $quantity;
    
    mysqli_query($con, "UPDATE stocks SET quantity='$new_quantity' WHERE name='$item'");
...
if (isset($_POST['checkout'])) {
    ...
    // Calculate totals
    $total_query = $con->query("SELECT SUM(subtotal) as total FROM debt_cart WHERE staffID='$staffID'");
    ...
    if ($order_result) {
        ...
        if ($balance > 0) {
            $outstand_query = $con->query("SELECT amount, balance FROM outstand WHERE customerID='$customerID'");
                              ...
                              <form method="POST">
                                  <div class="form-group">
                                      ...
                                      <select class="form-control basic" required name="speciality" onchange="getdoctor(this.value)">
                                          ...
                                          <?php
                                          $facilityID = $_SESSION['facilityID'];
                                          $ret = mysqli_query($con, "SELECT * FROM stocks WHERE quantity > 0");
                                  ...
                                  <div class="table-responsive">
                                      <table id="sample-table-1" class="table table-hover">
                                          ...
                                          <tbody>
                                              ...
                                              $staffID = $_SESSION['id'];
                                              $facilityID = $_SESSION['facilityID'];
                                              $sql = mysqli_query($con, "SELECT * FROM debt_cart WHERE customerID='$did' AND staffID='$staffID'");
                                              $cnt = 1;
